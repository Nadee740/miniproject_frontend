...
function MessExpenseList({suppliers,setSuppliers}) {
   ...
   useEffect(() => {
    ...
    if(user.designation=="faculty"){
        if(window.location.href.includes("HO")){
            ...
            axios.get(`${baseUrl}/warden/get-mess-expense?hostel=MH&&status=${1}&&isfaculty=${true}`)
            .then(res=>{
    ...
    //    else if(user.roles.includes("WD")){}
        ...
        axios.get(`${baseUrl}/warden/get-mess-expense?hostel=MH&&status=${0}&&isfaculty=${true}`)
        .then(res=>{
          setExpenseList(res.data.data)
          setLoading(false)
        })
    ...
    axios.get(`${baseUrl}/warden/get-mess-expense?hostel=MH&&status=${0}&&isfaculty=${false}`)
    .then(res=>{
      setExpenseList(res.data.data)
      setLoading(false)
    })
        ...
        {user.stage == "inmate" ? (
          <div className="flex items-center justify-between w-4/12">
            <select className="p-3 ring-slate-200 ring-2 rounded-xl outline-none">
              <option>
                {user.hostel == "MH" ? "Mens Hostel" : "Ladies Hostel"}
              </option>
            </select>
            {/* <select className='p-3 ring-slate-200 ring-2 rounded-xl outline-none'>
          ...
          {messexpenseList.map((expense, index) => {
            return (
              ...
              >
                ...
                <td className="p-3">{expense.particulars}</td>
  ...
  ):<EditPerDayMessExpense setIsEdit={setIsEdit} suppliers={suppliers} setSuppliers={setSuppliers} id={checkeditems[0]}/>;
}
