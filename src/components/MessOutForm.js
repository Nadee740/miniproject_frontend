...
function MessOutForm({noofDays,messOutHistory,setMessOutHistory,setIsEmpty,setNoofDays}) {
    ...
    useEffect(()=>{
      setLoading(true)      
    axios.post('http://localhost:8080/inmate/checkmessout',{
        user_id:user.user_id
    })
    .then((res)=>{
      setisMessOut(res.data.status)
      setMessoutFromdate(res.data.data.fromdate)   
      const today=new Date();
      const messoutfromdate=new Date(res.data.data.fromdate);
    ...
    const submitMessinForm = ()=>{
        setLoading(true)
        axios.post('http://localhost:8080/inmate/applymessin',{
                    user_id:user.user_id,
                    toDate:toDate,
                }).then((res)=>{
                    setMessOutHistory([...messOutHistory,res.data.rows[0]])
    ...
    const getMessinStartDate=()=>{
        ...
        today<messoutfromdate?date=messoutfromdate:date=today;
        let month = (date.getMonth() + 1).toString();
        let day = (date.getDate()+1).toString();
        let year = date.getFullYear();
        if (month.length < 2) {
        ...
        if(fdate.getTime()>tdate.getTime()){
            ...
            if(isMessout)
            {
                ...
                if(days<noofDays){
                    ...
                    setModalText("You have filled Mess in  from "+toDate+". Do you want to confirm?")
                    setOpen2(true)
                }
            }
            else{
    ...
    return (
        <div className='mb-3'>
            <h2 className='font-semibold text-lg mb-2'>{isMessout?'Apply for Mess in':'Apply for Mess Out'}</h2>
            <form onSubmit={submitHandler}>
                <div className='grid grid-cols-2 w-6/12 gap-4 mb-3'>
                    {isMessout?(<><label htmlFor="">From:</label> <input type="date" value={toDate} min={getMessinStartDate()} onChange={(e)=>{setToDate(e.target.value)}} className="w-full py-2 px-3 rounded-xl ring-2 ring-slate-300 focus:outline-none"  required/></>):(<><label htmlFor="">Period of Leave From:</label>  <input type="date" value={fromDate} min={getToday()} onChange={(e)=>{setFromDate(e.target.value)}} className="w-full py-2 px-3 rounded-xl ring-2 ring-slate-300 focus:outline-none" required/></>)}
            ...
            <ConfirmDialog open={open2} setOpen={setOpen2} modalHeading={modalHeading} modalText={modalText} confirmFunction={isMessout?submitMessinForm:submitForm}/>
